...
// export const getMoviesUpcoming = id => {
//   return fetch(
...
export const getMoviesReleased = () => {
  return fetch(
    `https://api.themoviedb.org/3/discover/movie?primary_release_date.gte=2020-01-01&primary_release_date.lte=2022-12-12`
  ).then((response) => {
    if (!response.ok) {
      throw new Error(response.json().message);
    }
...
export const getDetails = async () => {
  return fetch(
    "https://api.themoviedb.org/3/person/${id}/details/list?api_key=" +
      process.env.REACT_APP_TMDB_KEY +
      "&language=en-US"
  ).then( (response) => {
    if (!response.ok) {
